...
func (fh *forwarderHealth) init() {
	...
	apiKeyCount := 0
	for _, dr := range fh.keysPerDomains {
		apiKeyCount += len(dr.ApiKeys)
	}
...
func (fh *forwarderHealth) computeDomainsURL() {
	for domain, dr := range fh.keysPerDomains {
		apiDomain := ""
		...
		fh.keysPerAPIEndpoint[apiDomain] = append(fh.keysPerAPIEndpoint[apiDomain], dr.ApiKeys...)
	}
}
